...
- first typed serial command envelope:
  ...
  denies target/capability mismatches, bad-schema envelopes, or over-capable
  targets before dispatch without provider writes, candidate-byte loading,
  persistence, or broad mutation; accepted and denied envelope decisions retain
  current-boot local-only audit evidence visible through `audit.events`
