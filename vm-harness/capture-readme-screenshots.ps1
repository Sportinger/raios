param(
    ...
    [string]$Prompt = "Reply with exactly five words: raiOS boots, networks, chats directly.",
...
$RunId = "raios-readme-screenshots-{0:yyyyMMdd-HHmmss}-{1}" -f (Get-Date), $PID
$RunDir = Join-Path $env:TEMP $RunId
$TempImage = Join-Path $RunDir "raios-stage0-local-openai.img"
...
try {
    ...
    Capture-Screen -Name "raios-home"
    ...
    Capture-Screen -Name "raios-openai-chat"
    $Succeeded = $true
}
finally {
    if ($QemuPid) {
