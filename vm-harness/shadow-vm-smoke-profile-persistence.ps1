...
# Live shadow-20260719-170525-6196 measured storage.selftest at up to 89s.
Send-AgentCommand -Command "storage.selftest" -ExpectedMarker "RAIOS_STORAGE selftest=pass absent_grant=storage_capability_absent out_of_range=output_span_out_of_artstor quota_overflow=output_span_length_exceeds_lease ram_quota=nospc logged=1 persistent_effect=0 media_write_attempts=0 ram_unchanged=1" -Name "persistence:storage_selftest" -TimeoutSeconds 300
Send-AgentCommand -Command "storage.selftest" -ExpectedMarker "RAIOS_STORAGE disk=pass reclog_unchanged=1 artstor_unchanged=1" -Name "persistence:storage_disk_invariance" -TimeoutSeconds 300
