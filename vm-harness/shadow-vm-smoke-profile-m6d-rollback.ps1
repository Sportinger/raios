...
function New-DevPromotionSignatureHex {
    ...
    try {
        ...
        Add-Predicate -Name "m6d:dev_signature_host_generation_gap" -Expected "host can generate DER P-256 dev signature" -Passed $true -Actual $_.Exception.Message
        return $null
    }
}
...
Add-Predicate -Name "m6d:serial_delivery_retains_valid_echo_wasm" -Expected "runtime-delivered echo wasm retained in RAM" -Passed $deliveryOk -Actual $(if ($deliveryOk) { "matched" } else { ($result | ConvertTo-Json -Compress -Depth 6) })
if (-not $deliveryOk) {
    throw "Expected serial-delivered echo wasm to finalize as retained valid candidate"
}

$negativeOffset = Get-SerialLogOffset
Send-AgentCommand -Command "module.load_ephemeral svc.dev.granted_candidate" -ExpectedMarker "RAIOS_AGENT_END module.load_ephemeral" -Name "m6d:ungranted_load_denied_command"
...
Add-Predicate -Name "m6d:ungranted_candidate_denied_no_instantiation" -Expected "ungranted delivered candidate hits generic capability_denied and does not instantiate" -Passed $negativeOk -Actual $(if ($negativeOk) { "matched" } else { ($negativeLoad | ConvertTo-Json -Compress -Depth 6) })
if (-not $negativeOk) {
    throw "Expected ungranted candidate load to fail closed before instantiation"
}

Send-AgentCommand -Command "service.rollback_apply svc.dev.granted_candidate" -ExpectedMarker "RAIOS_AGENT_END service.rollback_apply" -Name "m6d:no_promotion_rollback_apply_denied_command"
$noPromotionRollback = Get-LastAgentResponseJson -Method "service.rollback_apply"
$noPromotionResult = $noPromotionRollback.body.result
$noPromotionOk = (
    $noPromotionRollback.t -eq "response" -and
    $noPromotionResult.code -eq "capability_denied" -and
    $noPromotionResult.reason -eq "no_recorded_promotion_to_roll_back" -and
    $noPromotionResult.trust_tier -eq "dev_key_not_owner_sealed" -and
    $noPromotionResult.owner_sealed -eq $false -and
    $noPromotionResult.durable -eq $false -and
    $noPromotionResult.persistence -eq "none"
)
Add-Predicate -Name "m6d:no_promotion_rollback_apply_fails_closed" -Expected "rollback_apply before a recorded promotion returns capability_denied without mutation" -Passed $noPromotionOk -Actual $(if ($noPromotionOk) { "matched" } else { ($noPromotionResult | ConvertTo-Json -Compress -Depth 8) })
if (-not $noPromotionOk) {
    throw "Expected no-promotion rollback_apply to fail closed"
}
...
Add-Predicate -Name "m6d:in_guest_selftest_positive_negative_and_rollback_gates" -Expected "selftest proves granted run, ungranted denial, hash-mismatch denial, live projection truth, rollback restore, no-promotion denial, and one-shot denial" -Passed $selftestOk -Actual $(if ($selftestOk) { "matched" } else { ($selftest.body.result | ConvertTo-Json -Compress -Depth 8) })
if (-not $selftestOk) {
...
Add-Predicate -Name "m6d:projection_selftest_loaded_truth" -Expected "loaded projection positives true, guardrails false, trust_tier dev_key_not_owner_sealed" -Passed $projectionLoadedOk -Actual $(if ($projectionLoadedOk) { "matched" } else { ($projectionLoadedCase | ConvertTo-Json -Compress -Depth 8) })
...
Add-Predicate -Name "m6d:projection_selftest_not_loaded_false" -Expected "not-loaded projection keeps every projected boolean false" -Passed $projectionAbsentOk -Actual $(if ($projectionAbsentOk) { "matched" } else { ($projectionAbsentCase | ConvertTo-Json -Compress -Depth 8) })
if (-not $projectionAbsentOk) {
    throw "Expected not-loaded live projection selftest case"
}

$rollbackRecordedCase = @($selftestCases | Where-Object { $_.case -eq "rollback_apply_recorded_promotion_restores_inventory" })[0]
$rollbackRecordedOk = (
    $rollbackRecordedCase.passed -eq $true -and
    $rollbackRecordedCase.capability_denied -eq $false -and
    $rollbackRecordedCase.rollback_restore_hash_verified -eq $true -and
    $rollbackRecordedCase.trust_tier -eq "dev_key_not_owner_sealed" -and
    $rollbackRecordedCase.durable -eq $false
)
Add-Predicate -Name "m6d:selftest_rollback_recorded_promotion_restores_inventory" -Expected "recorded promotion selftest verifies reprojected inventory hash restores pre-load baseline" -Passed $rollbackRecordedOk -Actual $(if ($rollbackRecordedOk) { "matched" } else { ($rollbackRecordedCase | ConvertTo-Json -Compress -Depth 8) })
if (-not $rollbackRecordedOk) {
    throw "Expected recorded-promotion rollback selftest to pass"
}

$rollbackNoPromotionCase = @($selftestCases | Where-Object { $_.case -eq "rollback_apply_no_promotion_denied" })[0]
$rollbackNoPromotionOk = (
    $rollbackNoPromotionCase.passed -eq $true -and
    $rollbackNoPromotionCase.capability_denied -eq $true -and
    $rollbackNoPromotionCase.trust_tier -eq "dev_key_not_owner_sealed" -and
    $rollbackNoPromotionCase.durable -eq $false
)
Add-Predicate -Name "m6d:selftest_rollback_no_promotion_denied" -Expected "rollback selftest fails closed when no promotion was recorded this boot" -Passed $rollbackNoPromotionOk -Actual $(if ($rollbackNoPromotionOk) { "matched" } else { ($rollbackNoPromotionCase | ConvertTo-Json -Compress -Depth 8) })
if (-not $rollbackNoPromotionOk) {
    throw "Expected no-promotion rollback selftest denial"
}

$rollbackOneShotCase = @($selftestCases | Where-Object { $_.case -eq "rollback_apply_one_shot_second_denied" })[0]
$rollbackOneShotOk = (
    $rollbackOneShotCase.passed -eq $true -and
    $rollbackOneShotCase.capability_denied -eq $false -and
    $rollbackOneShotCase.rollback_restore_hash_verified -eq $true -and
    $rollbackOneShotCase.second_rollback_apply_denied -eq $true -and
    $rollbackOneShotCase.trust_tier -eq "dev_key_not_owner_sealed" -and
    $rollbackOneShotCase.durable -eq $false
)
Add-Predicate -Name "m6d:selftest_rollback_one_shot_second_apply_denied" -Expected "rollback selftest proves successful apply is one-shot and second apply is denied" -Passed $rollbackOneShotOk -Actual $(if ($rollbackOneShotOk) { "matched" } else { ($rollbackOneShotCase | ConvertTo-Json -Compress -Depth 8) })
if (-not $rollbackOneShotOk) {
    throw "Expected one-shot rollback selftest denial"
}
...
if ($liveSignatureVerified) {
    ...
    Send-AgentCommand -Command "module.load_ephemeral svc.dev.granted_candidate" -ExpectedMarker "RAIOS_AGENT_END module.load_ephemeral" -Name "m6d:granted_candidate_load"
    ...
    $loadOk = (
        ...
        $loadResult.authorizes_rollback_install -eq $false -and
        $loadResult.owner_sealed -eq $false -and
        $loadResult.durable -eq $false -and
        $loadResult.persistence -eq "none" -and
        $loadResult.rollback_plan.present -eq $true
    )
    Add-Predicate -Name "m6d:granted_candidate_loads_ram_only_service_and_records_rollback_plan" -Expected "granted external candidate loads as dev-tier current_boot RAM service and records RAM rollback plan" -Passed $loadOk -Actual $(if ($loadOk) { "matched" } else { ($loadResult | ConvertTo-Json -Compress -Depth 7) })
    if (-not $loadOk) {
    ...
    Add-Predicate -Name "m6d:granted_candidate_starts_and_runs_wasm" -Expected "instantiation_ok=true run_outcome=success fuel_used>0 guest-log evidence" -Passed $startOk -Actual $(if ($startOk) { "fuel_used=$($run.fuel_used) log_line=$($run.log_line)" } else { ($startResult | ConvertTo-Json -Compress -Depth 7) })
    if (-not $startOk) {
    ...
    Add-Predicate -Name "m6d:granted_candidate_serial_guest_log" -Expected "WASM_GUEST_LOG serial line after granted start" -Passed $guestLogOk -Actual $(if ($guestLogOk) { "found_after_offset:$loadOffset" } else { Get-SerialLogTail -Path $SerialLog })
    if (-not $guestLogOk) {
        throw "Expected granted candidate run to emit WASM_GUEST_LOG"
    }

    Send-AgentCommand -Command "agent module.loader_runtime" -ExpectedMarker "RAIOS_AGENT_END module.loader_runtime" -Name "m6d:live_loader_runtime_projection"
    ...
    Add-Predicate -Name "m6d:live_loader_runtime_projection_reflects_granted_run" -Expected "live_granted_load_projection true while native loader readiness stays false" -Passed $loaderProjectionOk -Actual $(if ($loaderProjectionOk) { "matched" } else { ($loaderRuntime.body.result | ConvertTo-Json -Compress -Depth 6) })
    ...
    $rollbackOk = (
        $rollback.t -eq "response" -and
        $rollbackResult.code -eq "ok" -and
        $rollbackResult.action -eq "rollback_apply" -and
        $rollbackResult.service_id -eq "svc.dev.granted_candidate" -and
        $rollbackResult.trust_tier -eq "dev_key_not_owner_sealed" -and
        $rollbackResult.owner_sealed -eq $false -and
        $rollbackResult.durable -eq $false -and
        $rollbackResult.persistence -eq "none" -and
        $rollbackResult.loaded -eq $false -and
        $rollbackResult.running -eq $false -and
        $rollbackResult.rollback_verification.restore_hash_verified -eq $true
    )
    Add-Predicate -Name "m6d:granted_candidate_rollback_apply_restores_inventory_hash" -Expected "rollback_apply stops, clears, frees slot, removes service, and verifies pre-load inventory hash" -Passed $rollbackOk -Actual $(if ($rollbackOk) { "matched" } else { ($rollbackResult | ConvertTo-Json -Compress -Depth 8) })
    if (-not $rollbackOk) {
    ...
    if (-not $postRollbackInventoryOk) {
        throw "Expected service.inventory to remove granted candidate after rollback"
    }

    Send-AgentCommand -Command "service.rollback_apply svc.dev.granted_candidate" -ExpectedMarker "RAIOS_AGENT_END service.rollback_apply" -Name "m6d:second_rollback_apply_denied"
    $secondRollback = Get-LastAgentResponseJson -Method "service.rollback_apply"
    $secondRollbackResult = $secondRollback.body.result
    $secondRollbackOk = (
        $secondRollback.t -eq "response" -and
        $secondRollbackResult.code -eq "capability_denied" -and
        $secondRollbackResult.reason -eq "no_recorded_promotion_to_roll_back" -and
        $secondRollbackResult.trust_tier -eq "dev_key_not_owner_sealed" -and
        $secondRollbackResult.owner_sealed -eq $false -and
        $secondRollbackResult.durable -eq $false -and
        $secondRollbackResult.persistence -eq "none"
    )
    Add-Predicate -Name "m6d:second_rollback_apply_fails_closed_one_shot" -Expected "second rollback_apply after successful un-promote returns capability_denied" -Passed $secondRollbackOk -Actual $(if ($secondRollbackOk) { "matched" } else { ($secondRollbackResult | ConvertTo-Json -Compress -Depth 8) })
    if (-not $secondRollbackOk) {
        throw "Expected second rollback_apply to fail closed"
    }
}
else {
    Add-Predicate -Name "m6d:live_signature_positive_profile_gap" -Expected "live signed grant path available" -Passed $true -Actual "live signature did not verify; positive rollback proof is the in-guest selftest"
}
