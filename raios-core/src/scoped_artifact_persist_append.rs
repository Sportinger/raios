pub const SCOPED_ARTIFACT_PERSIST_APPEND_DECISION_SCHEMA: &str =
    "raios.scoped_artifact_persist_append_authorization_decision.v0";
pub const SCOPED_ARTIFACT_PERSIST_APPEND_DECISION_ID: &str =
    "scoped_artifact_persist_append_authorization.current_boot.seed_data.reclog.v0";
pub const SCOPED_ARTIFACT_PERSIST_APPEND_DECISION_MARKER: &str =
    "RAIOS_ARTIFACT_PERSIST_APPEND_SCOPE_DECISION";

pub const EXPECTED_METHOD: &str = "durable.artifact_persist_append";
pub const EXPECTED_TARGET_ID: &str = "append.artifact_persist.seed_data";
...
#[derive(Clone, Copy)]
pub struct ScopedArtifactPersistAppendInput<'a> {
...
impl<'a> ScopedArtifactPersistAppendInput<'a> {
...
pub struct ScopedArtifactPersistAppendDecision {
    pub performed: bool,
    pub status: &'static str,
    pub reason: &'static str,
}

pub fn evaluate_scoped_artifact_persist_append(
    input: &ScopedArtifactPersistAppendInput<'_>,
) -> ScopedArtifactPersistAppendDecision {
...
fn require_str(
    ...
    missing: &'static str,
    mismatch: &'static str,
) -> Result<(), ScopedArtifactPersistAppendDecision> {
    match actual {
...
fn denied(reason: &'static str) -> ScopedArtifactPersistAppendDecision {
    ScopedArtifactPersistAppendDecision {
        performed: false,
        status: "denied",
        reason,
...
mod tests {
    ...
    fn valid_input() -> ScopedArtifactPersistAppendInput<'static> {
        ScopedArtifactPersistAppendInput {
    ...
    #[test]
    fn performs_only_exact_artifact_persist_append_evidence() {
        assert_eq!(
            evaluate_scoped_artifact_persist_append(&valid_input()),
            ScopedArtifactPersistAppendDecision {
                performed: true,
                status: "appended",
                reason: "authorized_artifact_persist_append_readback_reparse_verified"
    ...
    fn empty_log_chain_expects_seq_one_and_zero_prev() {
        ...
        assert!(evaluate_scoped_artifact_persist_append(&input).performed);
    }
    ...
    fn apply(input: &mut ScopedArtifactPersistAppendInput<'static>, mutation: Mutation) {
        match mutation {
    ...
    fn denial_truth_table_names_first_failed_pin() {
        ...
        while idx < cases.len() {
            ...
            let decision = evaluate_scoped_artifact_persist_append(&input);
