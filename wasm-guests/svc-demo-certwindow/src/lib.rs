...
static mut OUT: [u8; raios_x509_time::CERTWINDOW_RECORD_LEN] =
    [0; raios_x509_time::CERTWINDOW_RECORD_LEN];
...
pub extern "C" fn raios_service_main() -> i32 {
    ...
    unsafe {
        ...
        // addr_of discipline (no &static mut): form the slice through a raw ptr.
        let cert: &[u8] = &(*addr_of!(IN))[..n];
        let result = raios_x509_time::parse_x509_cert_validity_window(cert);
        let encoded = raios_x509_time::encode_certwindow_record(result);
        (*addr_of_mut!(OUT)).copy_from_slice(&encoded);
        let out_ptr = addr_of!(OUT).cast::<u8>() as i32;
        output_write(out_ptr, raios_x509_time::CERTWINDOW_RECORD_LEN as i32);
    }
    0
}
