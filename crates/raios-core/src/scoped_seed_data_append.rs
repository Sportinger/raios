pub const SCOPED_SEED_DATA_APPEND_DECISION_SCHEMA: &str =
    "raios.scoped_seed_data_append_authorization_decision.v0";
pub const SCOPED_SEED_DATA_APPEND_DECISION_ID: &str =
    "scoped_seed_data_append_authorization.current_boot.seed_data.reclog.v0";
pub const SCOPED_SEED_DATA_APPEND_DECISION_MARKER: &str = "RAIOS_SEED_DATA_APPEND_SCOPE_DECISION";

pub const EXPECTED_METHOD: &str = "durable.record_log_append";
pub const EXPECTED_TARGET_ID: &str = "append.record_log.seed_data";
pub const EXPECTED_RECORD_SCHEMA: &str = "raios.durable_record.v0";
pub const EXPECTED_REGION_MARKER: &str = "RAIOS_DATA_RECLOG";
const SECTOR_SIZE: u64 = 512;

#[derive(Clone, Copy)]
pub struct ScopedSeedDataAppendInput<'a> {
...
impl<'a> ScopedSeedDataAppendInput<'a> {
...
pub struct ScopedSeedDataAppendDecision {
    pub performed: bool,
    pub status: &'static str,
    pub reason: &'static str,
}

pub fn evaluate_scoped_seed_data_append(
    input: &ScopedSeedDataAppendInput<'_>,
) -> ScopedSeedDataAppendDecision {
    ...
    ScopedSeedDataAppendDecision {
        performed: true,
        status: "appended",
        reason: "authorized_seed_data_reclog_append_readback_reparse_verified",
    }
}
...
fn require_str(
    ...
    missing: &'static str,
    mismatch: &'static str,
) -> Result<(), ScopedSeedDataAppendDecision> {
    match actual {
...
fn denied(reason: &'static str) -> ScopedSeedDataAppendDecision {
    ScopedSeedDataAppendDecision {
        performed: false,
        status: "denied",
        reason,
...
mod tests {
    ...
    fn valid_input() -> ScopedSeedDataAppendInput<'static> {
        ScopedSeedDataAppendInput {
    ...
    #[test]
    fn performs_only_exact_seed_data_append_evidence() {
        assert_eq!(
            evaluate_scoped_seed_data_append(&valid_input()),
            ScopedSeedDataAppendDecision {
                performed: true,
                status: "appended",
                reason: "authorized_seed_data_reclog_append_readback_reparse_verified"
    ...
    fn empty_log_chain_expects_seq_one_and_zero_prev() {
        ...
        assert!(evaluate_scoped_seed_data_append(&input).performed);
    }
    ...
    fn apply(input: &mut ScopedSeedDataAppendInput<'static>, mutation: Mutation) {
        match mutation {
            Mutation::MissingMethod => input.method = None,
            Mutation::WrongMethod => input.method = Some("durable.record_log_scan"),
            Mutation::MissingTarget => input.target_id = None,
            Mutation::WrongTarget => input.target_id = Some("append.audit_ledger.current_boot"),
            Mutation::MissingSchema => input.record_schema = None,
            Mutation::WrongSchema => input.record_schema = Some("raios.audit_record.v0"),
    ...
    fn denial_truth_table_names_first_failed_pin() {
        ...
        while idx < cases.len() {
            ...
            let decision = evaluate_scoped_seed_data_append(&input);
