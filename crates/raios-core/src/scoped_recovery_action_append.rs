pub const SCOPED_RECOVERY_ACTION_APPEND_DECISION_SCHEMA: &str =
    "raios.scoped_recovery_action_append_authorization_decision.v0";
pub const SCOPED_RECOVERY_ACTION_APPEND_DECISION_ID: &str =
    "scoped_recovery_action_append_authorization.current_boot.seed_data.reclog.v0";
pub const SCOPED_RECOVERY_ACTION_APPEND_DECISION_MARKER: &str =
    "RAIOS_RECOVERY_ACTION_APPEND_SCOPE_DECISION";

pub const EXPECTED_METHOD: &str = "durable.recovery_action_append";
pub const EXPECTED_TARGET_ID: &str = "append.recovery_action.seed_data";
...
#[derive(Clone, Copy)]
pub struct ScopedRecoveryActionAppendInput<'a> {
...
pub struct ScopedRecoveryActionAppendDecision {
    pub performed: bool,
    pub status: &'static str,
    pub reason: &'static str,
}

pub fn evaluate_scoped_recovery_action_append(
    input: &ScopedRecoveryActionAppendInput<'_>,
    ...
    if input.disable_target_core_owned {
        return denied("target_core_owned");
    }
...
fn require_str(
    ...
    missing: &'static str,
    mismatch: &'static str,
) -> Result<(), ScopedRecoveryActionAppendDecision> {
    match actual {
...
fn denied(reason: &'static str) -> ScopedRecoveryActionAppendDecision {
    ScopedRecoveryActionAppendDecision {
        performed: false,
        status: "denied",
        reason,
...
mod tests {
    ...
    fn valid_input() -> ScopedRecoveryActionAppendInput<'static> {
        ScopedRecoveryActionAppendInput {
    ...
    #[test]
    fn performs_only_exact_recovery_action_append_evidence() {
        assert_eq!(
            evaluate_scoped_recovery_action_append(&valid_input()),
            ScopedRecoveryActionAppendDecision {
                performed: true,
                status: "appended",
                reason: "authorized_recovery_action_append_readback_reparse_verified"
    ...
    fn empty_log_chain_expects_seq_one_and_zero_prev() {
        ...
        assert!(evaluate_scoped_recovery_action_append(&input).performed);
    }

    #[test]
    fn exactly_known_disablable_classification_authorizes() {
        // Baseline: only known_disablable is set true; authorizes.
        assert!(evaluate_scoped_recovery_action_append(&valid_input()).performed);

        // core_owned flips to its own row.
        let mut core_owned = valid_input();
        core_owned.disable_target_core_owned = true;
        assert_eq!(
            evaluate_scoped_recovery_action_append(&core_owned).reason,
            "target_core_owned"
        );

        // lifeline endpoint flips to its own row.
        let mut lifeline = valid_input();
        lifeline.disable_target_is_lifeline_endpoint = true;
        assert_eq!(
            evaluate_scoped_recovery_action_append(&lifeline).reason,
            "target_is_lifeline_endpoint"
        );

        // unknown target (not known-disablable) flips to its own row.
        let mut unknown = valid_input();
        unknown.disable_target_known_disablable = false;
        assert_eq!(
            evaluate_scoped_recovery_action_append(&unknown).reason,
            "target_unknown"
        );
    }
    ...
    fn apply(input: &mut ScopedRecoveryActionAppendInput<'static>, mutation: Mutation) {
        match mutation {
            Mutation::MissingMethod => input.method = None,
            Mutation::WrongMethod => input.method = Some("durable.promotion_transaction_append"),
            ...
            Mutation::WrongSchema => input.record_schema = Some("raios.promotion_transaction.v0"),
            ...
            Mutation::GrantsCapability => input.grants_new_capability = true,
    ...
    fn denial_truth_table_names_first_failed_pin() {
        let cases = [
            ...
            (Mutation::MissingActionKind, "missing_action_kind"),
            (Mutation::WrongActionKind, "action_kind_out_of_scope"),
