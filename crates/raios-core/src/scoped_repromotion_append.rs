pub const SCOPED_REPROMOTION_APPEND_DECISION_SCHEMA: &str =
    "raios.scoped_repromotion_append_authorization_decision.v0";
pub const SCOPED_REPROMOTION_APPEND_DECISION_ID: &str =
    "scoped_repromotion_append_authorization.current_boot.seed_data.reclog.v0";
pub const SCOPED_REPROMOTION_APPEND_DECISION_MARKER: &str =
    "RAIOS_REPROMOTION_APPEND_SCOPE_DECISION";

pub const EXPECTED_METHOD: &str = "repromotion.run";
pub const EXPECTED_TARGET_ID: &str = "append.repromotion_audit.seed_data";
pub const EXPECTED_RECORD_SCHEMA: &str = "raios.repromotion.v0";
pub const EXPECTED_REGION_MARKER: &str = "RAIOS_DATA_RECLOG";
pub const EXPECTED_TRUST_TIER: &str = "dev_key_not_owner_sealed";
const SECTOR_SIZE: u64 = 512;

#[derive(Clone, Copy)]
pub struct ScopedRepromotionAppendInput<'a> {
...
impl<'a> ScopedRepromotionAppendInput<'a> {
...
pub struct ScopedRepromotionAppendDecision {
    pub performed: bool,
    pub status: &'static str,
    pub reason: &'static str,
}

pub fn evaluate_scoped_repromotion_append(
    input: &ScopedRepromotionAppendInput<'_>,
...
fn require_str(
    ...
    missing: &'static str,
    mismatch: &'static str,
) -> Result<(), ScopedRepromotionAppendDecision> {
    match actual {
...
fn denied(reason: &'static str) -> ScopedRepromotionAppendDecision {
    ScopedRepromotionAppendDecision {
        performed: false,
        status: "denied",
        reason,
...
mod tests {
    ...
    fn valid_input(status: &'static str) -> ScopedRepromotionAppendInput<'static> {
        ScopedRepromotionAppendInput {
    ...
    #[test]
    fn authorizes_reverified_and_denied_repromotion_audit_records() {
        for status in ["reverified", "repromotion_denied"] {
            assert_eq!(
                evaluate_scoped_repromotion_append(&valid_input(status)),
                ScopedRepromotionAppendDecision {
                    performed: true,
                    status: "appended",
                    reason: "authorized_repromotion_audit_append_readback_reparse_verified"
    ...
    fn empty_log_chain_expects_seq_one_and_zero_prev() {
        ...
        assert!(evaluate_scoped_repromotion_append(&input).performed);
    }
    ...
    fn apply(input: &mut ScopedRepromotionAppendInput<'static>, mutation: Mutation) {
        match mutation {
            Mutation::MissingMethod => input.method = None,
            Mutation::WrongMethod => input.method = Some("durable.artifact_persist_append"),
            Mutation::MissingTarget => input.target_id = None,
            Mutation::WrongTarget => input.target_id = Some("append.artifact_persist.seed_data"),
            Mutation::MissingSchema => input.record_schema = None,
            Mutation::WrongSchema => input.record_schema = Some("raios.artifact_persist.v0"),
            Mutation::MissingMarker => input.region_marker = None,
            Mutation::WrongMarker => input.region_marker = Some("RAIOS_DATA_ARTSTOR"),
            ...
            Mutation::ReverifiedWithoutWould => input.would_repromote = false,
            Mutation::DenialWithWould => {
                input.decision_status = Some("repromotion_denied");
                input.would_repromote = true;
            }
    ...
    fn denial_truth_table_names_first_failed_pin() {
        let cases = [
            ...
            (
                ...
                "reverified_decision_missing_would_repromote",
            ),
            (
                Mutation::DenialWithWould,
                "denial_decision_cannot_would_repromote",
            ),
