...
Gleichzeitig ist der aktuelle Pfad noch kein SeedOS-Agent-Protokoll. Er ist ein
fest verdrahteter Prompt-Client mit Human-Console-Ausgabe. Es fehlen
`seedos.agent.v0`, `system.snapshot.v0`, Capability-Entscheidungen,
...
- `openai::build_request_body()` sendet nur einen freien Prompt. Es gibt keine
  ...
  `seedos.agent.v0`-Envelope und keine Korrelation von Provider-Antworten mit
  SeedOS-Protokollmethoden.
...
- `docs/invariant-choices.md` fordert fuer Networking "TLS sessions pinned via
  ...
  muss klar zwischen direktem Provider-Prompt-Pfad und SeedOS-Control-Plane
  unterscheiden.
...
- Hoch: Secrets bleiben laenger im Speicher als noetig. `provider_config`
  ...
  Local-Image-Pfad per `SEEDOS_DEFAULT_OPENAI_API_KEY` ist nuetzlich, muss aber
...
2. `docs/invariant-choices.md` praezisieren: WebSocket bleibt die
   SeedOS-Control-Plane fuer Fake-Cloud/OTA/Module, aber direkter
   ...
   strukturierte Envelope fuer SeedOS-Toolverkehr.
