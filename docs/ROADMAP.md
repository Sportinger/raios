...
and lifeline protocol emit extraction, and Shadow VM report evidence cleanup.
The recovery lifeline command
...
`release/vm-reports/shadow-20260524-073050-11832.json` recorded 2725/2725
...
- `cargo fmt --all -- --check` passed after extracting recovery lifeline command
  ...
  artifact selftest and lifeline protocol emit helpers.
- `powershell -NoProfile -ExecutionPolicy Bypass -File scripts\build-seed-kernel.ps1 -Profile release`
  ...
  command-effect type-surface helpers plus artifact selftest and lifeline
