...
basic input. Network and provider calls are not expected to work on real
hardware yet.
...
4. Try typing `help`.
5. If input is missing, try another USB port or enable firmware legacy USB
   keyboard support. If the machine has no PS/2-compatible path, SeedOS needs a
   USB xHCI/HID driver before it can be operated there.
