# ADR 0001: SeedOS Agent Protocol Instead Of Porting Codex CLI
...
Stage-0 SeedOS intentionally has far less: boot, framebuffer, serial logs, input, one network path, trust anchors, and a small event loop.

## Decision
Do not port or run the Codex CLI inside Stage-0.

Build a small SeedOS-native agent protocol instead. The device exposes explicit, capability-gated tools such as:
