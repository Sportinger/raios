...
open_resolved now attenuates ALL mounts monotonically, requested∩mount∩
parent.rights_inheriting), rustc parses, resolves std, type-checks, CODEGENS
(rmeta+object land under /out, out_files=2 out_bytes=3180 — the old
rmeta<rand> os76 wall is gone) and invokes rust-lld, which fails on the last
artifact: `rust-lld: error: cannot open output file /out/hello.wasm:
Capabilities insufficient` (735 rounds, exit 1, shadow-20260719-151342).
Earlier calibrations that got here: SYMLINK_FOLLOW no-op (os76→os69),
readonly attenuation (os69→E0463), O_DIRECTORY (E0463→os76-on-write),
O_TRUNC on writable arenas (438b6c6/8081c9b/1166ca6). FRONTIER EVIDENCE
(host hypotheses falsified, 28098c9): preopens install full writable rights
(base+inheriting), raw-preopen lld-style create/truncate is GREEN in host
tests (67/67), production exposes only the composite "/" preopen, and
Rights::from_bits accepts the whole preview1 bit range — so the live denial
depends on the guest's exact path_open arguments, not on any host-reproduced
shape. Next: read RAIOS_RUSTCDIAG's recent-call ring (fd+errno of the
failing open) from the quick+persist run, then a one-shot kernel capture of
the failing open's full args (path/oflags/rights) if still ambiguous — files
wasi_preview1.rs/wasi_build_job.rs. Honest scope: §6 "rustc compiles a real
program" stays OPEN — full compile work runs on-device, no hello.wasm yet.
