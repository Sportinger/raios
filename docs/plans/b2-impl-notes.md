...
- `project_store::load_revision` reads an immutable manifest by exact project/revision identity. The child route uses it after commit to reparse and hash-validate revision 1's source blobs, and the current-boot snapshot retains that readback result plus both lineage entries.
