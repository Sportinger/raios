...
#[rustfmt::skip]
pub(crate) fn emit_recovery_memory_write_authority_selftest_case(case: &RecoveryMemoryWriteAuthoritySelfTestCase, comma: bool) {
    emit_inline_record_object(vec![f("case", s(case.name)), f("expected_status", s(case.expected_status)), f("expected_reason", s(case.expected_reason)), f("actual_status", s(case.actual_status)), f("actual_reason", s(case.actual_reason)), f("passed", b(case.passed)), f("accepts_raw_command_body", no()), f("dispatches_lifeline_command", no()), f("writes_recovery_memory", no()), f("command_execution_enabled", no()), f("load_attempted", no())], comma);
}
