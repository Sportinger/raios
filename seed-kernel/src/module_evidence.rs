...
pub struct RecoveryLifelineCommandExecutorCapabilityTableHashInput<'a> {
    pub retained_command_dispatch_behavior_event_id: &'a str,
    pub command_id: &'a str,
    pub argument_schema: &'a str,
    pub argument_hash: [u8; 32],
    pub target_locator: &'a str,
...
pub struct RecoveryLifelineCommandSideEffectGateHashInput<'a> {
    pub retained_executor_capability_table_event_id: &'a str,
    pub command_id: &'a str,
    pub argument_schema: &'a str,
    pub argument_hash: [u8; 32],
    pub target_locator: &'a str,
...
pub fn computed_recovery_lifeline_command_executor_capability_table_hash(
    input: RecoveryLifelineCommandExecutorCapabilityTableHashInput<'_>,
) -> [u8; 32] {
    let mut hash = Sha256::new();
    hash_static_line(
        &mut hash,
        b"canonicalization=raios.recovery_lifeline_command_executor_capability_table.canonical.v0",
        true,
    );
    hash_static_line(
        &mut hash,
        b"schema=raios.recovery_lifeline_command_executor_capability_table.v0",
        true,
    );
    hash_static_line(&mut hash, b"load_mode=recovery_only", true);
    hash_static_line(&mut hash, b"subject=agent.session.serial", true);
    hash_static_line(
        &mut hash,
        b"resource=recovery_lifeline_command_executor_capability_table",
        true,
    );
    hash_static_line(&mut hash, b"scope=current_boot", true);
    hash_str_line(
        &mut hash,
        b"retained_command_dispatch_behavior_event_id",
        input.retained_command_dispatch_behavior_event_id,
        true,
    );
    hash_str_line(&mut hash, b"command_id", input.command_id, true);
    hash_str_line(&mut hash, b"argument_schema", input.argument_schema, true);
    ...
    hash_str_line(
        &mut hash,
        b"executor_capability_table_id",
        input.executor_capability_table_id,
        true,
    );
    hash_hash_line(
        &mut hash,
        b"executor_capability_projection_sha256",
        input.executor_capability_projection_hash,
        true,
    );
    hash_static_line(&mut hash, b"accepts_raw_command_body=false", true);
    hash_static_line(&mut hash, b"accepts_lifeline_command_body=false", true);
...
pub fn computed_recovery_lifeline_command_side_effect_gate_hash(
    input: RecoveryLifelineCommandSideEffectGateHashInput<'_>,
) -> [u8; 32] {
    let mut hash = Sha256::new();
    hash_static_line(
        &mut hash,
        b"canonicalization=raios.recovery_lifeline_command_side_effect_gate.canonical.v0",
        true,
    );
    hash_static_line(
        &mut hash,
        b"schema=raios.recovery_lifeline_command_side_effect_gate.v0",
        true,
    );
    hash_static_line(&mut hash, b"load_mode=recovery_only", true);
    hash_static_line(&mut hash, b"subject=agent.session.serial", true);
    hash_static_line(
        &mut hash,
        b"resource=recovery_lifeline_command_side_effect_gate",
        true,
    );
    hash_static_line(&mut hash, b"scope=current_boot", true);
    hash_str_line(
        &mut hash,
        b"retained_executor_capability_table_event_id",
        input.retained_executor_capability_table_event_id,
        true,
    );
    hash_str_line(&mut hash, b"command_id", input.command_id, true);
    hash_str_line(&mut hash, b"argument_schema", input.argument_schema, true);
...
pub fn computed_recovery_lifeline_command_execution_stage_hash(
    ...
    if let Some(execution_commit_gate_hash) = input.execution_commit_gate_hash {
        hash_hash_line(
            &mut hash,
    ...
    if let Some(execution_audit_denial_hash) = input.execution_audit_denial_hash {
        hash_hash_line(
            &mut hash,
    ...
    if let Some(execution_observation_denial_hash) = input.execution_observation_denial_hash {
        hash_hash_line(
            &mut hash,
